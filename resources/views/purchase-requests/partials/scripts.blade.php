...
<script>
    ...
    $(document).ready(function() {
        ...
        //Handle add form
        $('#create-pr, #edit-pr').submit(function(e) {
            e.preventDefault();
            let form = $(this);
            let isEdit = form.attr('id') === 'edit-pr';
            let url = isEdit ? `/purchase-request/${$('#edit_id').val()}` : "{{ route('pr.store') }}";
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.status === 200){
                        ...
                        $('#edit_id').val(pr.id);
                        $('#edit_pr_number').val(pr.pr_number);
                        $('#edit_requested_by').val(pr.requested_by);
                        $('#edit_status').val(pr.status);
