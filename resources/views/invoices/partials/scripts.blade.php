...
<script>
    ...
    $(document).ready(function() {
        ...
        //Handle add form
        $('#create-invoice, #edit-invoice').submit(function(e) {
            e.preventDefault();
            let form = $(this);
            let isEdit = form.attr('id') === 'edit-invoice';
            let url = isEdit ? `/invoice/${$('#edit_id').val()}` : "{{ route('invoice.store') }}";
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.status === 200){
                        ...
                        $('#edit_id').val(invoice.id);
                        $('#edit_invoice_number').val(invoice.invoice_number);
                        $('#edit_type').val(invoice.type);
                        $('#edit_ref_id').val(invoice.ref_id);
                        $('#edit_total').val(invoice.total);
        ...
        //     let id = $('#edit_id').val();
        //     let formData = new FormData(document.getElementById('edit-invoice'));
