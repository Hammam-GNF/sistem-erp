...
<script>
    ...
    $(document).ready(function() {
        ...
        //Handle add form
        $('#create-so, #edit-so').submit(function(e) {
            e.preventDefault();
            let form = $(this);
            let isEdit = form.attr('id') === 'edit-so';
            let url = isEdit ? `/sales-order/${$('#edit_id').val()}` : "{{ route('so.store') }}";
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.status === 200){
                        ...
                        $('#edit_id').val(so.id);
                        $('#edit_so_number').val(so.so_number);
                        $('#edit_customer_id').val(so.customer_id);
                        $('#edit_status').val(so.status);
